...
public class HoverGlow : MonoBehaviour {
	...
	[Tooltip("Si está activado, cambiará el color el objeto en función de la mano")]
	public bool useHover = true;

	[Tooltip("i está activado, el objeto utilizará su primaryHoverColor cuando se acerque la mano")]
	public bool usePrimaryHover = true;

	[Header("Colores")]
	// Hacemos una interpolación lineal entre dos colores para conseguir un color intermedio (Similar a un degradado)
	//en este caso concreto un gris muy oscuro
	public Color defaultColor = Color.Lerp(Color.black, Color.white, 0.1f);
	//en este caso concreto un girs mas claro
	...
	// Eeferencia al material del objeto
	...
	void Start(){
		...
		// Intentamos recuperar el componente Renderer ( ap artir del Renderer vamos a
	...
	void Update(){
		//Verificamos si existe una referencia al material
		if (material != null) {
			...
			// Si el evento detectado es el hover, el color objetivo será el definido comohoverColor
			} else {
				if (intObj.isHovered && useHover) {
					// Calculamos el glow en función de la distancia a la palma de la mano
					// convertimos con un Map el valor de 0 a 0.2 de forma proporcional de 1 a 2
